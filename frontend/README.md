...
- 기본 UI와 정책 정보: 로컬 `Pretendard Variable`(100–900)
- Figma 정책 화면의 하천 이름·예산: 로컬 `온글잎 박다현체`(400)

폰트 파일은 `src/assets/fonts`에서 함께 번들되므로 외부 폰트 CDN에 의존하지 않습니다. 이용 조건과 출처는 같은 폴더의 `FONT_NOTICES.md`를 확인하세요.
